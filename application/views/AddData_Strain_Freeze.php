...
<div class="box-header with-border">
  ...
  <div class="col-md-6">
    <span class="pull-right">
      <button class="btn btn-default" id = "butangBackFreeze"><i class="fa fa-arrow-left"> </i> Back</button>
    </span>
  </div>
</div>
...
$(document).ready(function () {
  ...
  $('#butangBackFreeze').unbind('click').click(function () {
    $.ajax({
  ...
  $('#location_detail_freeze').on('change', function() {
   ...
   $('#sumLokasi').empty().append(harun);
     ...
     $("form#dataFreeze").submit(function(){
      ...
      url = "<?php echo site_url('bacteria/test2')?>";
      ...
      $.ajax({
        ...
        success: function(json_data)
        {
          ...
          for (var i = 0; i < json_data.length; i++) {
                            ...
                            //alert(json_data[i].id);
                            var a = '<option value = '+json_data[i].row+'> '+json_data[i].row+' </option>';
...
$("#Strain_Global_ID_Freeze").focus(); //fokus bile start page
