...
<div class="box-header with-border">
  <div class="col-md-6">
    <h3 class="box-title">Edit Data Strain</h3>
  </div>
  ...
  <form class="form-horizontal" id="dataCryo" method="post" enctype="multipart/form-data">
    <input type="hidden" id = "jenis_Strain" value="<?php echo $data->Jenis_Strain; ?>" name="id"/> 
    <input type="hidden" id = "idStrain" value="<?php echo $data->id; ?>" name="idStrain"/> 
    ...
    <div class="box-body">
      ...
      <div class="form-group">
        ...
        <div class="col-sm-2">
          <h5 id = "sumLokasiCryo"><?php echo $data->Summary_Lokasi; ?></h5>
        </div>
      </div>
...
$(document).ready(function () {
  ...
  $('#butangBack').unbind('click').click(function () {
    $.ajax({
      url : "<?php echo site_url('Lokasi_controller/Load_Data_Matrix')?>/",
...
$("form#dataCryo").submit(function(){
  ...
  url = "<?php echo site_url('bacteria/update_Cryo_From_Matrix')?>";
      ...
      $.ajax({
        ...
        success: function(json_data)
        {
          ...
          for (var i = 0; i < json_data.length; i++) {
            ...
            var select = '';
            if(json_data[i].canister=='<?php echo $c ?>'){
              select = 'selected';
            }
      ...
      $.ajax({
        ...
        success: function(json_data)
        {
          ...
          for (var i = 0; i < json_data.length; i++) {
            ...
            var select = '';
            if(json_data[i].boxes=='<?php echo $d ?>'){
              select = 'selected';
            }
  ...
  success: function(json_data)
  {
    ...
    var b = '<option selected="selected">Please select</option>';
          ...
          for (var i = 0; i < json_data.length; i++) {
            ...
            var select = '';
            if(json_data[i].location_detail=='<?php echo $e ?>'){
              select = 'selected';
            }
...
$(document).ready(function(){
  ...
  $('#Lokasi_CanisterLoadAjax').on('change', function() {
    ...
    if (selboxes != "" && sellokasi != "") {
      ...
      $.ajax({
        url : "<?php echo site_url('Lokasi_controller/checkAvaibility_Cryo')?>/",
        type: "POST",
        data: {
          "canister" : a,
          "boxes" : b,
